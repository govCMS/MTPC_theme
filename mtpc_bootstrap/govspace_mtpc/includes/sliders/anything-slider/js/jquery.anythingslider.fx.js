        ...
        var wrap = $(this).closest('.anythingSlider'),
                ...
                getBaseFx = function(size) {
                    return {
                        ...
                        'bottom' : [
                            { inFx: { bottom: 0 }, outFx: { bottom: (size || sliderHeight) } }
                        ],
                        'left'   : [
                            { inFx: { left: 0 }, outFx: { left: '-' + (size || sliderWidth) } }
                        ],
                        'right'  : [
                            { inFx: { right: 0 }, outFx: { right: (size || sliderWidth) } }
                        ],
                        'fade'   : [
                            { inFx: { opacity: 1 }, outFx: { opacity: 0 } }
                        ],
                        'expand' : [
                            { inFx: { width: '100%', top: '0%', left: '0%' } , outFx: { width: (size || '10%'), top: '50%', left: '50%' } }
                        ],
                        'listLR' : [
            ...
            var baseFx = getBaseFx(), // get base FX with standard sizes
                ...
                // this hides element if out of the viewport (prevents captions - right & left only - from overlapping current window)
                    hideOffscreen = function(el) {
                        el.each(function() {
                            if (!$(this).closest('.panel').is('.activePage')) {
